# Lumen PHP Framework | MySQL - Api REST Control de gastos

1. Clonar repositorio

    ```bash
    git clone https://github.com/igmr/Lumen-Api-REST-ControlDeGastos.git
    ```

2. Descargar dependencias via composer

   ```bash
   cd Lumen-Api-REST-ControlDeGastos
   composer update
   ```

3. Ejecutar script de base de datos o manualmente crear tablas [Script SQL](#script-sql)

    ```bash
    mysql -u root -p < ./docs/script.sql
    ```

4. Renombrar el archivo `.env.example` por `.env` y configurar los parámetros de la base de datos
...
![Modelo Control de gastos](./docs/model.png)

```sql
-- ******************************************************************************
-- * DATABASE                                                                   *
-- ******************************************************************************
DROP DATABASE IF EXISTS lumen_api_rest_cost_control;
CREATE DATABASE IF NOT EXISTS lumen_api_rest_cost_control;
USE lumen_api_cost_control;
...
VALUES
    ...
    (013,"Cosméticos/Ciudado del cabello"    ,    NULL,    NULL,    NOW()),
